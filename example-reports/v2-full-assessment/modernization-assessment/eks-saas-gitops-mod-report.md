...
| **Gap** | No SSM Patch Manager configuration for automated OS patching. No AWS Inspector for continuous vulnerability assessment. No hardened AMI references (CIS benchmarks, Bottlerocket). Karpenter uses `al2023@latest` which is regularly updated but there's no automated node rotation to pick up new AMIs. Gitea uses AL2 (older) rather than AL2023. No container image signing or verification. |
