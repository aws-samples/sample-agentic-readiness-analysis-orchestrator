...
| **Recommendation** | Integrate container image scanning (Trivy or Amazon ECR image scanning) into the CI/CD pipeline. Enable Amazon Inspector for runtime vulnerability assessment on EKS (per preferences). Consider Bottlerocket-based AMIs for EKS nodes for additional OS-level hardening. The existing container security context is well-configured — maintain these practices. |
