...
**Date**: 2026-05-01
**ARA Report**: agentic-readiness-assessment/zg-cmp-atx-testing-portfolio-ara-report.md
**MOD Report**: modernization-assessment/zg-cmp-atx-testing-portfolio-mod-report.md
...
| ARA BLOCKERs Resolvable by MOD Phase 0 | 11 of 27 (41%) |
...
| 16 | DATA-Q1: Data Classification | BLOCKER | 14 of 34 | DATA-Q1: Data Classification (20 svc <2) + SEC-Q5: Secrets Mgmt (6 svc <2) | 20 | ARA extends MOD | Implement portfolio-wide data classification framework; ARA additionally requires field-level PII tagging and agent-specific redaction |
...
**Shared** (entries 5, 8–9, 11–14): Same underlying gap viewed through different assessment lenses. One remediation action resolves both the MOD and ARA findings. For example, OBS-Q1 (ARA Distributed Tracing) and OPS-Q1 (MOD Distributed Tracing) are the same capability gap — deploying X-Ray/ADOT resolves both. AUTH-Q5 (ARA Credential Management) and SEC-Q5 (MOD Secrets Management) both require migrating to AWS Secrets Manager.

**ARA extends MOD** (entry 16): ARA adds agent-specific requirements on top of what MOD already evaluates. MOD remediation (data classification framework, secrets management) partially addresses the ARA finding, but additional agent-specific work is needed — specifically field-level PII classification, sensitivity tagging on data entities, and classification-aware access controls that restrict agent queries to non-sensitive fields.

---

## Section 2: Agentic Readiness Delta

**If MOD Phase 0 were completed, 11 of 27 ARA BLOCKERs would be resolved.**

This means **41%** of the portfolio's agentic readiness blockers are actually modernization prerequisites —
...
|---|---|---|---|
| Machine Identity Authentication | AUTH-Q1 | 11 | 41% |
| Sensitive Data Classification | DATA-Q1 | 14 | 52% |
| Documented API Interface | API-Q1 | 2 | 7% |
| **Total** | | **27** | **100%** |
...
|---|---|---|
| DATA-Q1: Data Classification (14 services) | Alluxio--alluxio, apache--druid, conductor-oss--conductor, FlowiseAI--Flowise, Graylog2--graylog2-server, hapifhir--hapi-fhir, Lidarr--Lidarr, Prowlarr--Prowlarr, Radarr--Radarr, scality--cloudserver, Sonarr--Sonarr, thingsboard--thingsboard, ToolJet--ToolJet, umami-software--umami | ARA requires **field-level PII classification** and **agent-specific data redaction** — MOD Phase 0 addresses data governance at the platform level (DATA-Q1: data classification framework, SEC-Q5: secrets management) but does not implement the per-field sensitivity tagging, agent-aware query filtering, or classification-based access controls that ARA DATA-Q1 demands. The relationship is "ARA extends MOD" — MOD Phase 0 builds the foundation, but agent-specific data protection work remains. |
...
**Total requiring ARA-specific remediation**: 16 BLOCKERs (14 DATA-Q1 + 2 API-Q1)

### Delta Visualization

```
Total ARA BLOCKERs:          27 (100%)
├── Resolved by MOD Phase 0: 11 ( 41%)  ← AUTH-Q1 instances
└── Require ARA remediation: 16 ( 59%)  ← DATA-Q1 (14) + API-Q1 (2)
```

> **Key Insight**: The modernization dividend is concentrated entirely in the AUTH-Q1 (Machine Identity) BLOCKER. All 11 instances of AUTH-Q1 can be resolved as a side effect of MOD Phase 0 Security Foundation work. However, the largest BLOCKER category — DATA-Q1 (14 instances, 52% of all BLOCKERs) — requires ARA-specific remediation beyond what MOD provides. Teams should plan MOD Phase 0 first (resolving AUTH-Q1), then immediately follow with ARA DATA-Q1 remediation.
...
**Dual-Resolution Impact:** This single item resolves 2 MOD findings and 3 ARA findings across 28 services. Deploying centralized CloudWatch Logs, X-Ray/ADOT, and alerting simultaneously provides the MOD operational visibility and the ARA agent observability that both assessments require.
...
#### 7. Data Classification for Agent Access (DATA-Q1)

**ARA Finding:** DATA-Q1: Sensitive Data Classification (BLOCKER) — 14 services affected
**Why Not Fully Covered by MOD Phase 0:** MOD Phase 0 establishes a data classification framework (DATA-Q1 MOD) and secrets management (SEC-Q5), but ARA requires **field-level PII classification**, **agent-specific data redaction**, and **classification-aware access controls** that prevent agents from accessing `restricted` or `confidential` data fields. The MOD data governance provides the taxonomy; ARA extends it with per-field enforcement.
**Remediation Action:**
1. For each of the 14 affected services, audit the data model to identify PII, credentials, PHI, and business-sensitive fields
2. Apply sensitivity tags (`public`, `internal`, `confidential`, `restricted`) at the field/column level
3. Configure agent-accessible query endpoints to filter or redact fields classified as `confidential` or `restricted`
4. Implement deny-by-default for agent access to unclassified data
**Estimated Effort:** High (4–12 weeks per service)
**Dependencies:** MOD Phase 0 Security Foundation (provides secrets management baseline) and Data Classification framework
...
| 4 | OBS-Q1: Distributed Tracing | RISK-QUALITY (18 svc) | OPS-Q1: Distributed Tracing | 20 svc <2 | Shared | Deploy AWS X-Ray / ADOT across all services — identical finding in both assessments |
...
| 7 | ENG-Q1: Infra Governance | RISK-QUALITY (15 svc) | INF-Q10: IaC Coverage | 31 svc <2 | Shared | Standardize on AWS CDK or Terraform — identical finding in both assessments |
...
| # | ARA Finding | ARA Severity | MOD Finding | MOD Gap Services | Relationship | Sequencing Guidance |
|---|---|---|---|---|---|---|
| 17 | DATA-Q1: Data Classification | BLOCKER (14 svc) | DATA-Q1: Data Classification + SEC-Q5: Secrets Mgmt | 20 svc <2 | ARA extends MOD | Complete MOD data governance framework → then add field-level PII classification and agent-specific redaction |

### Deduplication Summary

- **17 findings** appear in both assessments (7 Shared + 9 MOD→ARA + 1 ARA extends MOD)
