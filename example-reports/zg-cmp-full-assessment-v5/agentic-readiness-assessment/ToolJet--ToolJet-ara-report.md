...
**Monorepo Note**: This is a monorepo. The assessment focuses on `server/` as the primary agent integration target (the NestJS API). The `frontend/` (React SPA), `plugins/` (library), `marketplace/` (library), and `cli/` (dev tool) are noted as supporting context but are not directly agent-callable.
