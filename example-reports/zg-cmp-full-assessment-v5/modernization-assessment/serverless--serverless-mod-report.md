...
| **Date** | 2025-07-16 |
| **TD Version** | (not available — run `atx custom def get -n modernization-assessment` to resolve) |
| **Repo Type** | monorepo |
...
|-------|-------|
| **Score** | 4 |
| **Finding** | Modern cloud-native stack: **JavaScript (ESM)** with Node.js 18+ (CI runs on 24.x), AWS SDK v3 (3.1015.0 — current as of assessment), Jest 30, esbuild 0.27.4, Express 5, zod v4, Zod-validated schemas throughout. **Go 1.26.1** for the binary installer. All packages use `"type": "module"` (ESM). Dependencies are current and well-maintained. The MCP server uses `@modelcontextprotocol/sdk ^1.27.1`. |
