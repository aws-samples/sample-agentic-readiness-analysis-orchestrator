...
|----------------|--------------------------------|-----------|
| Cassandra | **Amazon DynamoDB** (preferred) or Amazon Keyspaces | DynamoDB is preferred per assessment preferences. Keyspaces provides Cassandra-compatible API for minimal code changes. DynamoDB requires a new `StorageComponent` implementation but offers superior scaling and zero operational overhead. |
...
| MySQL | **Amazon Aurora MySQL** (preferred) or Amazon RDS MySQL | Aurora is preferred per assessment preferences. MariaDB JDBC driver is compatible with Aurora MySQL. Schema (`mysql.sql`) uses standard SQL with no stored procedures — direct migration path. |
...
| **Recommendation** | Deploy Zipkin on **Amazon EKS** (preferred per assessment preferences) using the existing Docker images. Create Kubernetes Deployment manifests, Service definitions, and HPA (Horizontal Pod Autoscaler) configuration. Alternatively, deploy on Amazon ECS with Fargate for a lower-operational-overhead option. |
