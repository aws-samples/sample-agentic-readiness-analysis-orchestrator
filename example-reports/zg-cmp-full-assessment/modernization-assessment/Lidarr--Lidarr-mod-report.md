...
| **Recommendation** | When containerizing for EKS, use hardened base images (e.g., .NET 8.0 runtime on Alpine or Bottlerocket nodes). Enable Amazon ECR image scanning for container vulnerability detection. Use AWS Systems Manager for node patching. Enable Amazon Inspector for continuous vulnerability assessment. |
