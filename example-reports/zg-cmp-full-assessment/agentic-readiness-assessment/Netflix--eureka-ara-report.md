...
**Monorepo Service Boundaries**: The primary deployable service is `eureka-server` (the discovery server). `eureka-client` is a library component embedded by other applications. Assessment focuses on eureka-server as the system agents would interact with.
