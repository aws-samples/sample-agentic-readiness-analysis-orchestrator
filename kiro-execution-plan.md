...
- [x] Clean working tree, create branch `feedback/td-reviewer-pass`.
- [x] Resolve B3 FIND uniqueness before edit: the "Archetype Calibration" string appears 4× in MOD TD. Use `#### INF-Q4:` header + preceding paragraph as anchor context.
- [x] Resolve INF-Q2 Score 2 duplicate: WF-1 wording supersedes P2A-2 for the Score 2 row. Apply once, not twice.
...
- [x] **M1 — "compute tiers" → "compute resource types"** (INF-Q7 Score 4 rubric row; 1 hit).
- [x] **M2 — "primary database" → "main production database"** in INF-Q8 Score 2 rubric row.
- [x] **M3 — "primary database" → "main production database"** in INF-Q9 Score 3 rubric row.
- [x] **M4 — "primary database" → "main production database"** in INF-Q2 Score 3 rubric row (WF-1 wording).

## Tier 2 — Discovery additions (rubric unchanged)

Look-for lists and question examples grow to match reality. Scoring unaffected.

- [x] **M5 — Remove App Mesh** from discovery list (line 192) and INF-Q6 look-for (line 714). End-of-support Sept 30, 2026.
- [x] **M6 — Add AppSync + IoT Core to INF-Q6** (question text + look-for).
- [x] **M7 — Add `appspec.yml` (CodeDeploy) to CI/CD discovery** (file pattern list + INF-Q11 look-for).
- [x] **M8 — Add MWAA to INF-Q3** workflow orchestration list (question text).
- [x] **M9 — Add Amazon MQ to INF-Q4** (question text + look-for).
- [x] **M10 — Add Neptune + Timestream to INF-Q2** (question text + look-for).
- [x] **M11 — Add observability/DR resources to INF-Q10 look-for** — CloudWatch alarms, Route 53 health checks, Backup plans. Addresses Robert's reopened C10.

**Commit 1 (Tier 1 + 2):** `chore(mod-td): terminology fixes and missing AWS services (P2B, P3, analysis-C10)` ✅ landed as `0cda661`

## Tier 3 — Scope + framing additions (non-rubric narrative)

One-line additions to Summary / why-it-matters. No rubric rows touched.

- [x] **M12 — Add AWS scope statement** above `This assessment does NOT cover:` (Summary). Signals on-prem/multi-cloud excluded.
- [x] **M13 — Name the 5 dimensions** on the `37 questions across 5 sections:` line (Summary). One-clause addition.
- [x] **M14 — Remove misplaced DMS/SCT reference** from INF-Q2 why-it-matters. Belongs in pathway detail, not scoring rationale.
- [x] **M15 — Add archetype-visibility note** above `**Archetype Calibration:**` in **INF-Q4 only** (not the other 3 occurrences). Makes the existing calibration visible to reviewers.
- [x] **M16 — Clarify INF-Q11 covers both application and IaC pipelines** (question text).
- [x] **M17 — Add S3 access-pattern sentence to DATA-Q1 why-it-matters.**
- [x] **M18 — Add S3 File Gateway note to DATA-Q1 Score 2 rubric row.**
- [x] **M19 — Broaden "containerize as-is" in APP-Q2 why-it-matters** → "containerize, migrate to serverless (Lambda), strangler fig extraction, or full decomposition."

**Commit 2 (Tier 3):** `docs(mod-td): clarify AWS scope, name dimensions, expand DATA-Q1 and APP-Q2 framing (P1C, P4)` ✅ landed as `870a67a`
...
- [x] **M20 — INF-Q1 Score 4** — "80%+ of compute" → service-count-based criterion ("≤1 EC2-based service remains").
- [x] **M21 — INF-Q2 Score 2** — apply merged WF-1 wording (supersedes P2A-2).
- [x] **M22 — INF-Q7 Scores 3 and 2** — custom vs default scaling policy sophistication.
- [x] **M23 — APP-Q2 Scores 3 and 2** — schema separation + circular dependency signals.
- [x] **M24 — APP-Q5 Scores 3 and 2** — define "inconsistent" as <50% adoption or conflicting schemes.
- [x] **M25 — SEC-Q3 Scores 3 and 2** — auth method vs coverage differentiation.
- [x] **M26 — SEC-Q3 Score 4** — carve-out for intentional public APIs.
- [x] **M27 — SEC-Q5 Scores 3 and 2** — differentiate by what's in env vars (non-secret vs production credential).
- [x] **M28 — SEC-Q7 Scores 2 and 1** — eliminate overlap. Score 2 = at least one tool; Score 1 = nothing.

**Commit 3 (Tier 4):** `refactor(mod-td): sharpen scoring rubric precision (P2A)` ✅ landed as `4b6f32e`
...
- [x] **M29 — APP-Q1 tone fix.** Rewrite Score 1/2 rubric rows to attribute gaps to AWS tooling limits, not label the customer "not modern." Keep modernization framing (this is the counter to the rejected agent-framework reframe). Robert flagged P0.
...
- [x] **M30 — SEC-Q2 Score 2/3 overlap.** Rework to differentiate on key type AND coverage, not coverage alone. Robert reopened.
- [x] **M31 — INF-Q9 Score 2/3 multi-AZ wording.** Distinguish stateless-recoverable single-AZ compute from stateful single-AZ compute. Robert reopened.
- [x] **M32 — SEC-Q2 Score 4 addition.** Extend to require centralized key management + rotation for customer-managed keys. Addresses Maia Thread 44. *(Merged into M30 rewrite.)*
- [x] **M33 — DATA-Q3 Score 4 addition.** Add documented version-update procedure (downtime awareness, risk acknowledgment). Addresses Maia Thread 41.
- [x] **M34 — INF-Q8 Score 4 extension.** Add cross-region backup replication as Score 4 signal. Addresses Maia Thread 29 (multi-region).
- [x] **M35 — OPS-Q9 Score 4 + look-for.** Remove SCP as tagging recommendation. Replace with IaC enforcement + Tag Policies + AWS Config rules. SCPs reserved for security measures. Addresses Maia Thread 53.

**Commit 4 (Tier 5):** `refactor(mod-td): address reopened reviewer concerns + Herrera Soto pass (APP-Q1 tone, SEC-Q2, INF-Q9, DATA-Q3, INF-Q8, OPS-Q9)` ✅ landed as `762e61d`
...
- [x] **M36 — INF-Q7 question broadening.** "auto-scaling mechanisms configured for compute workloads" → "...for compute, database, and other workloads."
- [x] **M37 — INF-Q7 why-it-matters.** Add sentence covering DynamoDB auto-scaling, Aurora Serverless scaling.
- [x] **M38 — INF-Q7 look-for.** Add DynamoDB auto-scaling, Aurora auto-scaling config to detection patterns.
- [x] **M39 — INF-Q7 Score 3 and 2 rubric rewrite.** Expand beyond "primary compute" wording so rubric matches the broadened question. Without this, the rubric contradicts the question text.

**Commit 5 (Tier 6):** `refactor(mod-td): broaden INF-Q7 auto-scaling across compute + data (P4-3 four-part edit)` ✅ landed as `0ea6062`
