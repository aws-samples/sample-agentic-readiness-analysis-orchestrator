...
> **The Execution Plan is generated by Claude, not by ct.** It uses `atx custom def exec` with `additionalPlanContext` via `-g file://atx-config-exec-plan.yaml`. This is the ONE place where `additionalPlanContext` is still used and relevant — ct handles ARA/MODA with no custom context (built-in types reject `-g`), but Claude handles EBA generation with full `additionalPlanContext`.
>
> **Claude's role:** Dynamically generate `atx-config-exec-plan.yaml` from discovered repos and ct findings, then run the EBA TD.
...
Claude gathers this automatically:
...
Use the Bash tool's `timeout` parameter directly.
