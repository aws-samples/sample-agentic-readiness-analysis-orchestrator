...
| `downstream_dependencies` | no | array | For `bpmn-opportunity` — downstream systems whose ARA reports get cross-referenced |
...
| `bpmn-opportunity` | Identifying agentic candidates in BPMN 2.0 process models. Requires `.bpmn` files in repos. |
| `full` | All three. Sequenced ARA → MOD → BAO within each repo, parallelized across repos. Produces a Bridge report when `portfolio_bridge` is configured. |
