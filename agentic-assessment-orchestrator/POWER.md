...
| `{name}-report.metadata.json` | Tiny sidecar carrying `{assessment_type, assessment_date, td_version}`. Downstream consumers (webapp, Bridge TD, future specs) can read this to identify the TD that produced the report. The same fields are redundantly embedded at the root of the main JSON under `metadata` so consumers that skip the sidecar still have access. |
