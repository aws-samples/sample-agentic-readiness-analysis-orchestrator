...
A delta counts as real only past `max(2·sd, floor)` — the noise floor (**ARA 0.09**, **MOD 0.04**) is a lower bound the measured `sd` can raise but never lower. A jittery fixture is held to a stricter bar than the floor; a quiet one is not held to a looser one, because an n=4 `sd` is far too weak an estimator to justify shrinking the bar and shrinking it is the direction that manufactures false improvements. **Below the threshold means NOT MEASURED — never "proven equal".**
