...
| **Recommendation** | Add Trivy or Amazon ECR image scanning to the CI/CD pipeline for container vulnerability scanning. Add `govulncheck` for Go dependency vulnerability detection. When migrating to EKS (preferred), use Bottlerocket AMIs for node hardening and Amazon Inspector for continuous vulnerability analysis. |
