# BPMN Agentic Opportunity Assessment: Camunda Invoice

**Repository**: `agentic-readiness-assessment/example-reports/bao-demo/repos/camunda-invoice`
...
> **Note:** Data readiness could not be assessed from the BPMN models. All agent tasks are classified as "data first" pending an Agentic Readiness Assessment (ARA) on the target systems. The two `businessRuleTask` candidates (Check decision, evaluate decision table) are strong Wave 1 candidates if ARA confirms data readiness.
...
|------|-----------|---------------|-------------|
| Check decision | DMN decision table inputs | Unknown — no `dataStoreReference` in BPMN, no ARA assessment | Run ARA on the DMN decision service. Business rule tasks typically consume structured inputs — likely ready. |
| evaluate decision table | DMN decision table inputs | Unknown — no `dataStoreReference` in BPMN, no ARA assessment | Run ARA on the DMN decision service. Business rule tasks typically consume structured inputs — likely ready. |
...
Not applicable — no ARA findings available to identify system-level blockers. This wave will be populated after ARA assessments are completed.
...
*Report generated by BPMN Agentic Opportunity Assessment. Classification scores are deterministic (computed from BPMN topology and scoring rules). Opportunity categories and autonomy levels are derived from the classification model described in the transformation definition.*
