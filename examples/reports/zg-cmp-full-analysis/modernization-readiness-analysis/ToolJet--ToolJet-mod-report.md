...
| **Gap** | Vulnerability scanning exists (Grype for Docker images, npm audit for dependencies) but compute is not hardened. Debian base image is standard, not security-hardened. No OS-level patching strategy. No Inspector for continuous analysis. |
| **Recommendation** | Switch Docker base image to **Alpine** or **Distroless** for reduced attack surface. Enable **AWS Inspector** for continuous vulnerability analysis. For EC2 deployments, use hardened AMIs (CIS benchmarks or Bottlerocket). Add SSM Patch Manager for OS patching. |
