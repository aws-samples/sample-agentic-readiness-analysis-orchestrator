...
> Note: AUTH-Q5 (Credential Management) is classified as RISK in the ARA framework but is not assigned to the RISK-SAFETY or RISK-QUALITY tier. For this analysis it is counted as INFO since hardcoded Google Maps API keys in a frontend demo template are informational for read-only scope. AUTH-Q5 is listed under INFOs in this report.
