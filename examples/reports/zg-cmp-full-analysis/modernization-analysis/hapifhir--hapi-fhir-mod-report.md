...
| **Recommendation** | For production deployment, use Amazon EKS with Bottlerocket nodes (hardened, minimal OS) or Fargate (no OS patching required). Use ECR image scanning for the production Docker image. Enable AWS Inspector for runtime vulnerability analysis. |
