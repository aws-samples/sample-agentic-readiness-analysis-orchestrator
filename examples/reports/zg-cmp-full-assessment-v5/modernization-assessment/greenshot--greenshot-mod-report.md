...
> **Note**: Greenshot is a Windows desktop application, not a cloud-native service. Many assessment questions evaluate cloud infrastructure maturity — VPCs, managed databases, auto-scaling, CloudTrail, distributed tracing — which are not applicable to the desktop application model. The low overall score reflects the absence of cloud infrastructure, not a failure to implement it where it would be appropriate. The assessment is most actionable in the areas of CI/CD maturity (INF-Q11), security pipeline (SEC-Q7), testing (OPS-Q6), and application language modernization (APP-Q1).
