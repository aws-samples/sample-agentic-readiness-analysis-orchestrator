...
> **INFO — Dev-Library-Application Override Applied**: This repository is classified as `repo_type: application` but functions as a build-system toolkit (library/CLI). The `service_archetype` is `stateless-utility` and all 5 surface flags are `false`. Per the ARA dev-library-application override, surface-flag downgrades are applied throughout the analysis. The original `repo_type` value (`application`) is preserved; this override affects scoring only.
