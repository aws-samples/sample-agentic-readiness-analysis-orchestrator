...
- **You must publish the remediation TD yourself — there is no built-in containerization transform.** ARA/MOD findings are assessment-only (`fix: null`), so `--ids` can never remediate them. The documented path is remediation's **direct-TD mode** (`--transformation-name` + `--repo`), which per the [official docs](https://docs.aws.amazon.com/transform/latest/userguide/continuous-modernization.html) runs *"a transformation against a repository without findings"* — so it needs no findings at all. `00-full-setup.sh` publishes `containerize-service` (Step 2); `03-remediate.sh` runs it.
  ```bash
  export MIDWAY=false                       # see the tenant note below — this is not optional
  ...
  # documented path (Console-visible, creates a branch/PR):
  atx ct remediation create --repo <src>::<repo> --source <src> \
    --transformation-name containerize-service --name containerize --local
  ```
- **Publish with `MIDWAY=false`, or remediation can't see your TD.** The registry is tenanted by authentication mode — one endpoint, two namespaces, and a TD published in one is a 404 in the other. The `ct remediation` worker always runs in AWS-credentials mode (`MIDWAY=false detected, using AWS credentials` in `~/.aws/atx/logs/debug*.log`), but a plain `custom def publish` may default the other way depending on your environment. Result: publish succeeds, `custom def get` confirms the TD, and `remediation create` still fails *"not found in the registry."* Setting `MIDWAY=false` on both the publish and the verify makes them agree; publishing to one namespace does not backfill the other, so re-publish rather than wait. Full write-up: `orchestrator/references/troubleshooting.md` → *"not found in the registry."*
  If it still won't resolve, `03-remediate.sh --path exec` runs the same TD via `atx custom def exec` — no registry round-trip, changes left uncommitted in the working tree.
