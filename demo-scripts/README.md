...
- **You must publish the remediation TD yourself — there is no built-in containerization transform.** ARA/MOD findings are assessment-only (`fix: null`), so `--ids` can never remediate them. The documented path is remediation's **direct-TD mode** (`--transformation-name` + `--repo`), which per the [official docs](https://docs.aws.amazon.com/transform/latest/userguide/continuous-modernization.html) runs *"a transformation against a repository without findings"* — so it needs no findings at all. `00-full-setup.sh` publishes `containerize-service` (Step 2); `03-remediate.sh` runs it.
  ...
  ```
- **Publish with `MIDWAY=false`, or remediation can't see your TD.** The registry is tenanted by auth mode — one endpoint, two namespaces. With Builder Toolbox on PATH (any Amazon dev laptop), `custom def publish` defaults to the **Midway** tenant, while the `ct remediation` worker logs `MIDWAY=false detected, using AWS credentials` and reads the **account** tenant. Result: publish succeeds, `custom def get` confirms the TD, and `remediation create` still fails *"not found in the registry"* — the symptom that cost a day on 2026-08-04 across three accounts before the cause was found. `MIDWAY=false` on both the publish and the verify makes them agree; publishing to one tenant does not backfill the other. Full write-up: `orchestrator/references/troubleshooting.md` → *"not found in the registry."*
